...
int mpu_init(struct int_param_s *int_param)
{
    unsigned char data[6];
            ...
            printf("[SENSOR] Full reset of DMP and FIFO...\n");
            
            // 1. 禁用DMP和FIFO
            unsigned char data1 = 0;
            i2c_write(st.hw->addr, 0x6A, 1, &data1);  // USER_CTRL = 0
            
            // 2. 复位FIFO和DMP
            data1 = 0x0C;  // BIT_FIFO_RST | BIT_DMP_RST
            i2c_write(st.hw->addr, 0x6A, 1, &data1);
            delay_ms(50);
            
            // 3. 清除中断使能
            data1 = 0;
            i2c_write(st.hw->addr, 0x38, 1, &data1);  // INT_ENABLE = 0
            
            // 4. 清除FIFO使能
            data1 = 0;
            i2c_write(st.hw->addr, 0x23, 1, &data1);  // FIFO_EN = 0
